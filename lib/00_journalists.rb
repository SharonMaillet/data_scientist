...
def shortest(arr)
    short = arr.sort_by(&:length)[0]
    puts "Le nom le plus court est #{short}"
end

def five_char(arr)
    no_at_arr = arr.map{|handle| handle[1..-1]}
    no_at_handle = no_at_arr.select{|five_char_handle| five_char_handle.length == 5}
    no_at_handle_length = no_at_handle.length
    puts "Il y a #{no_at_handle_length} handle contenant 5 charactères"
end

def maj(arr)
    arr_first_letter = arr.map{|handle| handle[1]}    
    start_maj = arr_first_letter.count{|i| i.match(/[A-Z]/)}
    
    puts "Il y #{start_maj} handles commencant par une majuscule"
end
...
def sort_by_length(arr)
    by_length = arr.sort_by(&:length)
    puts by_length
end

def epenser(arr)
    index_epenser = arr.index("@epenser") + 1
    puts "Le handle d'E-Penser est à la #{index_epenser}éme position"
end

def num_length(arr)
    only_lenght =arr.map{|i| i.length - 1}
    lenght_tally = only_lenght.tally

    lenght_tally.each do |key, value|
        puts "Il y a #{value} handle ayant #{key} charactères "
    end
end
